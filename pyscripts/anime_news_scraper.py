...
class animeNewsScraper(object):
	def anime_news_network(self):
		try:
			...
			for row in soup.findAll('div', attrs = {'class': 'herald box news'}):
				...
				image = ''
				try:
					image = 'https://animenewsnetwork.com' + row.find('div', attrs = {'class': 'thumbnail lazyload'})['data-src']
				except:
					image = ''
				...
				try:
					self.cards.append({'image': image, 'link': link, 'heading': text, 'news_provider': 'AnimeNewsNetwork', 'news_provider_link': URL})
				except:
					self.cards = [{'image': image, 'link': link, 'heading': text, 'news_provider': 'AnimeNewsNetwork', 'news_provider_link': URL}]
				ctr += 1
		except:
			print("ann failed")
		...
		try:
			...
			for row in soup.findAll('article', attrs = {'class': 'c-post-grid-item'}):
				...
				try:
					...
					image = ''
				link = row.find('a', attrs = {'class':'c-post-grid-item__link'})['href']
				text = row.h2.a.text
				card = {'image':image, 'link': link, 'heading': text, 'news_provider': 'AnimeUKNews', 'news_provider_link': URL}
				try:
	...
	def asahi_shimbun(self):
		try:
			...
			ctr = 0
			list_of_articles = soup.find('ul', attrs = {'class':'List ListHeadline'})
			for row in list_of_articles.findAll('li'):
				...
				for i in row.a.findAll('span'):
					i.clear()
				text = row.a.text
				card = {'image':image, 'link': link, 'heading': text, 'news_provider': 'AsahiShimbun', 'news_provider_link': URL}
				try:
					self.cards.append(card)
				except:
					self.cards = [card]
		...
		try:
			...
			for row in soup.findAll('article', attrs = {'class': 'browse-clip'}):
				if ctr == 10:
					break
				try:
					image = row.picture.find('source', attrs = {'sizes': '456px'})['srcset']
				except:
					try:
				...
				text = row.h3.a['title']
				card = {'image':image, 'link': link, 'heading': text, 'news_provider': 'CBR', 'news_provider_link': URL}
				try:
					self.cards.append(card)
				except:
					self.cards = [card]
	...
	def manga_tokyo(self):
		try:
			...
			for row in soup.findAll('article'):
				...
				link = row.h3.a['href']
				text = row.h3.a.text
				card = {'image':image, 'link': link, 'heading': text, 'news_provider': 'MANGA.TOKYO', 'news_provider_link': URL}
				try:
					self.cards.append(card)
				except:
					self.cards = [card]
